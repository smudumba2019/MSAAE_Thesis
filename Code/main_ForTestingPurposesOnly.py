...
def PlotMultipleTrips(TripInfo, ac, cruiseAltitudeInFeet, Show = True, Save = False, PlotGraphs = False):
    ...
    if Show == True:
        Chicago.ShowMap()
    return (Chicago.CLAP, Chicago.flightTime, Chicago.energyConsumed)
    
def PlotCLAP(TripInfo, ac):
    altitude = np.linspace(1500,7500,30) # desired cruising altitude(s) in feet - it is in an array to be able to plot CLAP vs cruising altitudes
    CLAP = []
    
    for h in altitude:
        clap, fltTime, eneConsumed = PlotMultipleTrips(TripInfo, ac, h, False)
...
def PlotPolar(Joby, FP1, cruiseAltitudeInFeet, Heading, TripD, distance, direction, aerodromeType):
    ...
    for t in range(len(distance)-1): 
        ...
        plt.tight_layout()
        number_str = str(t)
        plt.savefig('C:/Users/saimu/Documents/AAE_MS_Thesis_Documentation_v2/Python_Code/Results/Compass_Map/step' + number_str.zfill(4) + '.png', dpi=100)