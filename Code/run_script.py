...
def PlotAllCLAP(altitude_Joby, CLAP_Joby, altitude_Lilium, CLAP_Lilium, altitude_Archer, CLAP_Archer, altitude_Volocopter, CLAP_Volocopter, altitude_Ehang, CLAP_Ehang):
    ...
    plt.savefig('C:/Users/saimu/Documents/AAE_MS_Thesis_Documentation_v2/Python_Code/Results/AllCLAPvsAltRevised.png', dpi=600)
    plt.show()
...
def RunScript(TripInfo, CAF, Option, SaveMap = False, GenerateMapVideo = False, GeneratePolarPlotVideo = False, PlotGraphs = False):
    ...
    if Option == 1: # IF YOU WANT TO PLOT FOR A SPECIFIC CRUISING ALTITUDE, RUN THIS CODE
        CLAP_Joby_atCAF, FltTime_Joby, EneConsumed_Joby = PlotMultipleTrips(TripInfo, Joby, CAF[0], False, False, PlotGraphs) 
        CLAP_Lilium_atCAF, FltTime_Lilium, EneConsumed_Lilium = PlotMultipleTrips(TripInfo, Lilium7, CAF[1], False, False, PlotGraphs) 
        CLAP_Archer_atCAF, FltTime_Archer, EneConsumed_Archer = PlotMultipleTrips(TripInfo, Archer5, CAF[2], False, False, PlotGraphs) 
    ...
    try:
        ...
        return ((CLAP_Joby_atCAF, FltTime_Joby, EneConsumed_Joby),(CLAP_Lilium_atCAF, FltTime_Lilium, EneConsumed_Lilium),(CLAP_Archer_atCAF, FltTime_Archer, EneConsumed_Archer))
...
# RunScript((DepType, DEP[0], ArrType, ARR[0]), CAF, 1, GenerateMapVideo = False, GeneratePolarPlotVideo = False, PlotGraphs = True)
...
for dep in DEP: # example: DEP = [7, 9, 13, 9]
    for arr in ARR: # example: ARR = [8, 6, 7, 11]
        ...
        TripsFlightTime.append([Joby_Data[1], Lilium_Data[1], Archer_Data[1]])
        TripsEnergyConsumed.append([Joby_Data[2], Lilium_Data[2], Archer_Data[2]])
        AerodromeKeys.append([FromTo])

# RunScript((DepType, DEP[0], ArrType, ARR[0]), CAF, 1, GenerateMapVideo = False, GeneratePolarPlotVideo = False, PlotGraphs = False)
...
df.to_excel (r'C:/Users/saimu/Documents/AAE_MS_Thesis_Documentation_v2/TripsCruiseAltitudeFloor_Dataframe.xlsx', index = False, header=True)

TripsFlightTime_numpy = np.array(TripsFlightTime)
df1 = pd.DataFrame(TripsFlightTime_numpy, columns=['Joby','Lilium','Archer'])
df1.to_excel (r'C:/Users/saimu/Documents/AAE_MS_Thesis_Documentation_v2/TripsFlightTimes_Dataframe.xlsx', index = False, header=True)

TripsEnergyConsumed_numpy = np.array(TripsEnergyConsumed)
df2 = pd.DataFrame(TripsEnergyConsumed_numpy, columns=['Joby','Lilium','Archer'])
df2.to_excel (r'C:/Users/saimu/Documents/AAE_MS_Thesis_Documentation_v2/TripsEnergyConsumed_Dataframe.xlsx', index = False, header=True)

Trips_numpy = np.array(AerodromeKeys)
df3 = pd.DataFrame(Trips_numpy, columns=['TripsInfo'])
df3.to_excel (r'C:/Users/saimu/Documents/AAE_MS_Thesis_Documentation_v2/Trips_Dataframe.xlsx', index = False, header=True)
