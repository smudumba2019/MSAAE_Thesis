...
simTime = 90 # seconds - how long do we want to simulate?

def StartleResponseAnalysis(AC, cruiseAltitude):
    """
    ...
    X, Y, ETA, DistanceRadial = AC.ReachableGroundFootprint(cruiseAltitude,35,0) # X, Y are in meters, ETA and DistanceRadial are the equivalent polar coordinates angle and radius
    RGF_Area_Ref = Polygon(zip(X, Y)) # Assuming the OP's x,y coordinates
    RGF_Area_Ref = RGF_Area_Ref.area/(1000**2) # in km^2
    print(f"The Reachable Ground Footprint (RGF) Area for {cruiseAltitude} ft cruising altitude is: {RGF_Area_Ref} km^2")
    ...
    for t in timeDelay:
        ...
        X, Y, ETA, DistanceRadial = AC.ReachableGroundFootprint(AltitudeDrop,35,0)
...
timeDelay_JobyS4, AltitudeLoss_JobyS4, Loss_RGF_Area_JobyS4, PercentLoss_RGF_Area_JobyS4 = StartleResponseAnalysis(JobyS4, cruiseAltitude)

Lilium7 = Aircraft("Lilium", 7, 186, 186, 16.3, 60, 1700, 187.8, S=10.7*1.7)
timeDelay_Lilium7, AltitudeLoss_Lilium7, Loss_RGF_Area_Lilium7, PercentLoss_RGF_Area_Lilium7 = StartleResponseAnalysis(Lilium7, cruiseAltitude)

Archer5 = Aircraft("Archer", 5, 175, 60, 11.3, 24, 3175, 160, S=10.7*1.7)
timeDelay_Archer5, AltitudeLoss_Archer5, Loss_RGF_Area_Archer5, PercentLoss_RGF_Area_Archer5 = StartleResponseAnalysis(Archer5, cruiseAltitude)

Volocopter1 = Aircraft("Volocopter", 1, 70.6, 22, 2.5, 19, 900, 83.3, S=1.7*1.7)
timeDelay_Volocopter1, AltitudeLoss_Volocopter1, Loss_RGF_Area_Volocopter1, PercentLoss_RGF_Area_Volocopter1 = StartleResponseAnalysis(Volocopter1, cruiseAltitude)

EHang1 = Aircraft("Joby", 1, 62, 22, 1.5, 21, 1322, 206.2, S=10.7*1.7)
timeDelay_EHang1, AltitudeLoss_EHang1, Loss_RGF_Area_EHang1, PercentLoss_RGF_Area_EHang1 = StartleResponseAnalysis(EHang1, cruiseAltitude)
...
ax[1].set_xticks(np.arange(0,simTime,5))
...
ax[0].set_xticks(np.arange(0,simTime,5))
...
ax.set_xticks(np.arange(0, simTime, 5))
ax.set_ylabel('Altitude Loss (ft)')
ax.set_yticks(np.arange(-cruiseAltitude-200, 200, 100))
ax.grid(which='major', linestyle='-', linewidth='0.25', color='black')
plt.show()
