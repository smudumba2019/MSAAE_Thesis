...
class Aircraft:
    ...
    def ReachableGroundFootprint(self, altitude, speed, mu_deg, PowFailure):
        ...
        PSI = np.linspace(0, 3*math.pi/2, 100) # original PSI
        # PSI = np.linspace(0, math.pi, 100) # modified PSI
        ...
        X, Y = self.WindAnalysis(alt, speed, X, Y, 20, 0.01) # altitude, groundspeed, X, Y, wind speed, wind direction to North Po
        Distance = [math.sqrt(X[i] ** 2 + Y[i] ** 2) for i in range(len(X))]
        # print(Distance)
