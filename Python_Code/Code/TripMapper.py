...
class TripMapper(LoadExistingUAMaerodromeInfrastructure):
    ...
    def MapperInfrastructure(self):
        if self.MapType == "Satellite":
            ...
            p = figure(x_range=(-9780000-100000, -9745000+100000), y_range=(5050000, 5160000),x_axis_type="mercator", y_axis_type="mercator", title="Chicago Metropolitan Area", plot_width=1875, plot_height=910)
            # p = figure(x_range=(-9780000-55000, -9745000+15000), y_range=(5130000, 5160000),x_axis_type="mercator", y_axis_type="mercator", title="Chicago Metropolitan Area", plot_width=1200, plot_height=600)
            p.add_tile(tile_provider)
    ...
    def DrawNonDirectRoutingTrip(self, DepType, ArrType, idxDep, idxArr, Aircraft, CruiseAltitudeInFeet, WayPoints, Save=False):
        ...
        # FIND GEODESIC TRIP DISTANCE IN MILES
        geodesicDistance = self.GeodesicDistance(latDep_deg, lonDep_deg, latArr_deg, lonArr_deg)
        ...
        # LIST OF LONGITUDES ARE STORED IN 0TH INDEX AND LATITUDES ARE STORED IN 1ST INDEX
        lonWP_inDeg = WayPoints[0] 
        latWP_inDeg = WayPoints[1]
        howManyWaypoints = len(lonWP_inDeg) # find the number of waypoints that the user has defined

        for w in range(howManyWaypoints): # insert the wapoints into a dedicated list variable
        ...
        X_newArray = [] # nested lists [[],[],[]] of the reachable ground footprints in X-axis
        Y_newArray = [] # nested lists [[],[],[]] of the reachable ground footprint in Y-axis
        
        LatArrayforAllWaypoints_inMerc = [] # between each waypoint, add the steps of some size and store them as nested lists [[],[],[]]
        LonArrayforAllWaypoints_inMerc = [] # between each waypoint, add the steps of some size and store them as nested lists [[],[],[]]
        LatArrayforAllWaypoints_inDeg = [] # between each waypoint, add the steps of some size and store them as nested lists [[],[],[]]
        LonArrayforAllWaypoints_inDeg = [] # between each waypoint, add the steps of some size and store them as nested lists [[],[],[]]
        TripHeadingArrayforAllWaypoints_inDeg = [] # for each step in the waypoints, store the trip heading direction as a list []
        ...
        for l in range(len(LonWaypoints_inMerc)-1):
            Lat_inDeg, Lon_inDeg, TripHeading_deg = self.WaypointsConnector((LatWaypoints_inDeg[l], LonWaypoints_inDeg[l]),(LatWaypoints_inDeg[l+1], LonWaypoints_inDeg[l+1]),100)
        ...
        print(sum(TripDistanceNonDirectRouteLargerArray))
        ...
        FP1, FP1_time, E1 = self.PlotFlightProfiles(Aircraft,CruiseAltitudeInFeet)
        self.flightTime = FP1_time
        self.energyConsumed = E1
        ...
        for m in range(len(self.LatArrayforAllWaypoints_inMerc_flattened)): # go through each point in trip and find footprint for each point according to its flight profile
            ...
            if TripDistanceRel2Dep <= (0.621371*FP1.dx/1000) or TripDistanceRel2Dep >= self.TripDistanceNonDirectRoute - (0.621371*(FP1.dx)/1000):
                ...
                x_altitude, speed = FP1.GivenRangeOutputSpeed(TripDistanceRel2Dep)
                # speed = Aircraft.cruiseSpeed * 0.44704 # mph to m/s
                z = z * 3.28084 # in ft
                X, Y, Radial, FootprintDistance = Aircraft.ReachableGroundFootprint(z, speed, 45,0)
                FootprintDistance = FootprintDistance * 0.621371 # km to miles
                altitude_ft = z
            else:
                altitude_ft = CruiseAltitudeInFeet
                speed = Aircraft.cruiseSpeed * 0.44704 # m/s
                X, Y, Radial, FootprintDistance = Aircraft.ReachableGroundFootprint(CruiseAltitudeInFeet, speed, 45,0)
                FootprintDistance = FootprintDistance * 0.621371 # km to miles
        ...
        # FIND CLAP
        self.ComputeCLAP_v2(X_newArray, Y_newArray, Distance2ClosestContingencySite, Direction)
    ...
    def PlotFlightProfiles(self,Aircraft,CruiseAltitudeInFeet):
        ...
        FP2 = FlightProfile(Aircraft, 1500, self.tripDistance)
        ...
        self.FLT_ref = FP2_time
        self.EC_ref = E2
        
        if self.plotgraphs == True:
            ...
            ax.set_ylabel("Altitude Above Ground Level (feet)")
            ...
            plt.tight_layout()
            plt.savefig('C:/Users/saimu/OneDrive - purdue.edu/Purdue Graduate School/MS_Aeronautical_Astronautical_Engineering/MS Thesis Research/Spring 2022/Python_Code/Results/Miscellaneous/FlightProfileDirectRoute.png', dpi=600)
        # plt.show()
        return (FP1, FP1_time, E1)
    ...
    def FindClosestUAMaerodromeAlongTheRoute(self):
        ...
        for j in range(len(self.LatArrayforAllWaypoints_inMerc_flattened)): # j represents the each step of the vehicle along the path
            ...
            for l in range(len(self.lat_heliports_deg)): # l represents each location index of heliports
                ...
                Distance2ContingencySite_Heliport.append(10000000000000000000000)
        ...
        if self.plotgraphs == True:
            ...
            ax1.set_title("Distance, Type, and Heading Direction \nof Contingency Landing Site Relative To True North Pole")
            ...
            ax3.grid()
            plt.savefig('C:/Users/saimu/OneDrive - purdue.edu/Purdue Graduate School/MS_Aeronautical_Astronautical_Engineering/MS Thesis Research/Spring 2022/Python_Code/Results/Miscellaneous/HeadingDirectionVsTripLength_nonDirect.png', dpi=600)
    ...
    def ComputeCLAP_v2(self, X_newArray, Y_newArray, Distance2ClosestContingencySite, Direction):
        counter = 0 # count how many times the aircraft reachable footprint is outside the contingency landing sites
        sampSize = len(self.LatArrayforAllWaypoints_inMerc_flattened)
        
        for i in range(sampSize): # i represents each step along the route
            X_new = X_newArray[i] # in meters
            Y_new = Y_newArray[i] # in meters
            Lat_inMerc = self.LatArrayforAllWaypoints_inMerc_flattened[i]
            Lon_inMerc = self.LonArrayforAllWaypoints_inMerc_flattened[i]
            Lat_inDeg = self.LatArrayforAllWaypoints_inDeg_flattened[i]
            Lon_inDeg = self.LonArrayforAllWaypoints_inDeg_flattened[i]
            
            coords = [] # [(x0, y0), (x1, y1), ...] this will be used for finding if a point lies inside a polygon
            for j in range(0, len(X_new)): # for each point in the reachable footprint, convert it to coordinates from m to deg, create a tuple and store it in coords
                lat_deg, lon_deg = self.Mercator2Degrees((Lat_inMerc+Y_new[j]),(Lon_inMerc-X_new[j]))
                coords_tuple = (lat_deg, lon_deg)
                coords.append(coords_tuple)    
            poly = Polygon(coords) # now we have a polygon of the reachable ground footprint
        
            # now we can test to check if a polygon contains any of the contingency landing sites
            # FOR DEPARTURES
            latDep_deg_reg = list(self.lat_regional_deg)
            lonDep_deg_reg = list(self.lon_regional_deg)
            latDep_deg_maj = list(self.lat_major_deg)
            lonDep_deg_maj = list(self.lon_major_deg)
            latDep_deg_heli = list(self.lat_heliports_deg)
            lonDep_deg_heli = list(self.lon_heliports_deg)
            
            """ comment out the code below to include/exclude heliports """
            points_lat = [latDep_deg_reg, latDep_deg_maj, latDep_deg_heli]
            points_lon = [lonDep_deg_reg, lonDep_deg_maj, lonDep_deg_heli]
            # points_lat = [latDep_deg_reg, latDep_deg_maj]
            # points_lon = [lonDep_deg_reg, lonDep_deg_maj]            
            points_lat = list(chain.from_iterable(points_lat))
            points_lon = list(chain.from_iterable(points_lon))

            for k in range(len(points_lat)): # for each landing site point, check if it is in polygon
                point = Point(points_lat[k], points_lon[k])
                if poly.contains(point):
                    counter += 1
                    break
        print(f'Contingency Landing Assurance Percentage : {100*counter/sampSize} %')
        self.CLAP = 100-100*counter/sampSize
    ...
    def WaypointsConnector(self,LatLonFrom_merc, LatLonTo_merc, sampSize):
        ...
        if LonFrom_inMerc >= LonTo_inMerc and LatFrom_inMerc >= LatTo_inMerc:
            LonArray_inMerc = LonArray_inMerc[::-1]
            LatArray_inMerc = LatArray_inMerc[::-1]
    ...
    def SaveMap(self, Lat, Lon, imgNum, PatchLatj, PatchLonj):
        ...
        q.line(self.LatArrayforAllWaypoints_inMerc_flattened, self.LonArrayforAllWaypoints_inMerc_flattened, color = 'black', line_width = 2)
        q.circle(Lat, Lon, color = 'yellow', alpha = 1, size = 3)
        q.patch(PatchLatj, PatchLonj, alpha = 0.2, color="blue") # not this one
        ...
        # filename="C:/Users/saimu/Documents/AAE_MS_Thesis_Documentation_v2/Python_Code/Results/Trip_Map/test" + number_str.zfill(4) + ".png"
        filename = "C:/Users/saimu/OneDrive - purdue.edu/Purdue Graduate School/MS_Aeronautical_Astronautical_Engineering/MS Thesis Research/Spring 2022/Python_Code/Results/Trip_Map/test" + number_str.zfill(4) + ".png"
    ...
    def PlotContingencyLandingAssurance_vs_CruiseAltitude(self, altitude, CLAP):
        ...
        plt.savefig('C:/Users/saimu/Documents/MSAAE_Thesis_Code/Images/CLAPvsAlt.png', dpi=300)
        plt.show()
